...
plt.tight_layout()
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_training_losses_and_metrics.png', dpi=300, bbox_inches='tight')
plt.show()
...
###################################################################################################################
#
# TEMPORAL STRUCTURE ANALYSIS - Critical diagnostic for time series quality
#
###################################################################################################################
print("\n" + "="*80)
...
def analyze_temporal_patterns(original_data, generated_data):
    """Quick temporal structure analysis"""
    print(f"\nOriginal data length: {len(original_data)}")
    ...
    # Calculate rolling volatility (absolute values)
    window_size = min(50, len(original_data) // 4)
    orig_abs = np.abs(original_data)
    gen_abs = np.abs(generated_data[:len(original_data)])  # Match lengths
    
    orig_vol = pd.Series(orig_abs).rolling(window_size).std().dropna()
    gen_vol = pd.Series(gen_abs).rolling(window_size).std().dropna()
    
    # Compare first vs last quartiles
    q1_size = len(orig_vol) // 4
    orig_early_vol = orig_vol[:q1_size].mean()
    orig_late_vol = orig_vol[-q1_size:].mean()
    gen_early_vol = gen_vol[:q1_size].mean()
    gen_late_vol = gen_vol[-q1_size:].mean()
    
    print(f"Original volatility: Early={orig_early_vol:.6f}, Late={orig_late_vol:.6f}")
    print(f"Generated volatility: Early={gen_early_vol:.6f}, Late={gen_late_vol:.6f}")
    print(f"Original trend: {((orig_late_vol - orig_early_vol) / orig_early_vol * 100):+.2f}%")
    print(f"Generated trend: {((gen_late_vol - gen_early_vol) / gen_early_vol * 100):+.2f}%")
    
    # 2. Autocorrelation analysis
    print("\n2. AUTOCORRELATION ANALYSIS:")
    print("-" * 40)
    
    def simple_autocorr(x, lag):
    ...
    for lag in [1, 2, 5, 10]:
        orig_acf = simple_autocorr(original_data, lag)
        gen_acf = simple_autocorr(generated_data, lag)
        print(f"Lag {lag:2d}: Original={orig_acf:+.4f}, Generated={gen_acf:+.4f}, Diff={abs(orig_acf - gen_acf):.4f}")
    ...
    time_gen = np.arange(len(generated_data))
    
    slope_orig, _, r_orig, p_orig, _ = stats.linregress(time_orig, original_data)
    slope_gen, _, r_gen, p_gen, _ = stats.linregress(time_gen, generated_data)
    
    print(f"Linear trend (slope per time unit):")
    print(f"  Original: {slope_orig:.8f} (p-value: {p_orig:.4f})")
    print(f"  Generated: {slope_gen:.8f} (p-value: {p_gen:.4f})")
    
    # 4. Visual comparison of patterns
    ...
    orig_range = np.max(orig_sample) - np.min(orig_sample)
    gen_range = np.max(gen_sample) - np.min(gen_sample)
    
    print(f"Range comparison (first {n_compare} points):")
    print(f"  Original range: {orig_range:.6f}")
    print(f"  Generated range: {gen_range:.6f}")
    print(f"  Range ratio: {gen_range/orig_range:.3f}")
    
    # Check for obvious patterns
    orig_std_early = np.std(orig_sample[:n_compare//2])
    orig_std_late = np.std(orig_sample[n_compare//2:])
    gen_std_early = np.std(gen_sample[:n_compare//2])
    gen_std_late = np.std(gen_sample[n_compare//2:])
    
    print(f"Standard deviation comparison:")
    print(f"  Original: Early={orig_std_early:.6f}, Late={orig_std_late:.6f}")
    print(f"  Generated: Early={gen_std_early:.6f}, Late={gen_std_late:.6f}")
    
    # 5. Summary assessment
    print("\n5. TEMPORAL STRUCTURE ASSESSMENT:")
    print("-" * 40)
    
    # Check if generated data shows similar temporal patterns
    orig_vol_trend = (orig_late_vol - orig_early_vol) / orig_early_vol
    gen_vol_trend = (gen_late_vol - gen_early_vol) / gen_early_vol
    
    trend_similarity = 1 - abs(orig_vol_trend - gen_vol_trend) / (abs(orig_vol_trend) + 1e-8)
    
    if abs(orig_vol_trend) > 0.1 and abs(gen_vol_trend) < 0.05:
        print("❌ CRITICAL ISSUE: Original data shows strong temporal trend, generated data does not!")
        print("   This suggests the generator is producing noise-like data without temporal structure.")
    elif trend_similarity > 0.7:
        print("✅ Good: Generated data shows similar temporal trends to original")
    else:
        print("⚠️  Warning: Generated data shows different temporal patterns than original")
    
    print(f"Temporal trend similarity score: {trend_similarity:.3f} (1.0 = perfect)")
    
    return trend_similarity

# Run the temporal analysis
trend_score = analyze_temporal_patterns(OD_log_delta_np, fake_OD_log_delta_np)
...
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_original_vs_generated_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
axes[1].grid()
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_acf_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.tight_layout()
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_time_series_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.grid(True)
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_cdf_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
# Show and save the plots
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_original_vs_generated_timeseries.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_pdf_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_detailed_acf_comparison.png', dpi=300, bbox_inches='tight')
plt.show()
...
plt.grid(True)
plt.savefig('/Users/shawngibford/dev/Pennylane_QGAN/qGAN/qgan_histogram_comparison.png', dpi=300, bbox_inches='tight')
plt.show()

# Print summary of all saved plots
print("\n" + "="*80)
print("📊 QGAN PLOTS SAVED TO FILES")
print("="*80)
print("✅ qgan_training_losses_and_metrics.png - Training losses and temporal metrics over epochs")
print("✅ qgan_original_vs_generated_comparison.png - Side-by-side comparison of original vs generated time series")
print("✅ qgan_acf_comparison.png - Autocorrelation function comparison")
print("✅ qgan_time_series_comparison.png - Time series comparison with matching scales")
print("✅ qgan_cdf_comparison.png - Cumulative distribution function comparison")
print("✅ qgan_original_vs_generated_timeseries.png - Original vs generated time series plots")
print("✅ qgan_pdf_comparison.png - Probability density function comparison")
print("✅ qgan_detailed_acf_comparison.png - Detailed autocorrelation function analysis")
print("✅ qgan_histogram_comparison.png - Histogram comparison of distributions")
print("\nAll plots have been saved to the qGAN directory for viewing and analysis!")
print("="*80)
